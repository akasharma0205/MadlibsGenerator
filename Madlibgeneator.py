...
def Story1(win):
  ...
  NewScreen = Toplevel(win, bg='darkgreen')
  ...
  SubmitButton = Button(NewScreen, text="Submit", background="bisque", font=('Times', 12), command=lambda:final(NewScreen, Name.get(), game.get(), city.get(), player.get(), drink.get(), snack.get()))
...
def Story2(win):
    def final(tl: Toplevel, profession, noun, feeling, emotion,verb):
          ...
          feeling '''
            ...
            Label(tl, text='Story:',  wraplength=tl.winfo_width()).place(x=160, y=310)
            Label(tl, text=text,wraplength=tl.winfo_width()).place(x=0, y=330)
    NewScreen = Toplevel(win, bg='bisque')
    ...
    SubmitButton = Button(NewScreen, text="Submit", background="lightskyblue", font=('Times', 12), command=lambda:final(NewScreen, Profession.get(), Noun.get(), Feeling.get(), Emotion.get(), Verb.get()))
    SubmitButton.place(x=150, y=270)
